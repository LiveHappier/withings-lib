...
client.apiCall = function (url, method, cb) {
    ...
    if (method === 'get') {
        console.log("will log signed url");
...
// Get intradays activity
client.getIntraDayActivity = function (start_date,end_date, cb) {
    var params = {
        startdate: moment(start_date).unix()
        
    };
    if(end_date){
        params.enddate = end_date;
    }else{
        params.enddate = moment().add(23, 'hours').unix();
    }
    this.get('measure', 'getintradayactivity', params, cb);
};
